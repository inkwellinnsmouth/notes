...
## Core Directives
1. **Adhere to the Bible:** Every prompt must reinforce the project's visual rules (`Theme.md`): monotone black and white, heavy charcoal sketch texture, pervasive fog, visible film grain, pie-cut eyes, and floating limbs. Do not deviate. All generated keyframes MUST be strictly 16:9 aspect ratio and completely free of any text, lettering, words, or speech bubbles.
...
3. **Utilize Higgsfield AI Best Practices:**
   ...
   - **Leverage 'Soul' / Consistent References:** For generating the **Start Frame**, use the approved character reference image (e.g., `characters/barker_blot/images/barker_blot_front.png`). For the **End Frame**, explicitly instruct the user to use their newly generated *Start Frame* as the image-to-image reference, prompting only for the intended modifications to prevent scene jumps.
...
2. **Keyframe Generation (Nano banana):** 
   ...
   - Instruct the user to generate the End Frame, ensuring they use their newly generated Start Frame as the input reference image (rather than the raw character sheet) and apply the modifying End Frame prompt.
